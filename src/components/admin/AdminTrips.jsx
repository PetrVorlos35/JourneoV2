...
const categoryLabels = {
  ...
  accommodation: { label: 'Ubytování', emoji: '🏨' },
  food: { label: 'Jídlo', emoji: '🍽️' },
  activities: { label: 'Aktivity', emoji: '🎯' },
...
const AdminTrips = () => {
  ...
  const handleDeleteTrip = async (tripId, title) => {
    const ok = await confirmDialog({
      title: 'Smazat výlet?',
      message: `Opravdu chcete smazat výlet "${title}"? Budou smazány i všechny aktivity, výdaje a dokumenty.`,
    ...
    try {
      ...
      toast.success('Výlet byl smazán.');
      if (selectedTrip?.id === tripId) setSelectedTrip(null);
    } catch (err) {
      toast.error(err.message || 'Chyba při mazání výletu.');
    }
  };
  ...
  const handleViewTrip = async (tripId) => {
    ...
    try {
      ...
      toast.error('Nepodařilo se načíst detail výletu.');
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-2 text-[11px] text-gray-500">
                        ...
                        <span className="flex items-center gap-1"><DollarSign size={12} /> {t.totalExpenses.toLocaleString('cs-CZ')} Kč</span>
                        <span className="flex items-center gap-1"><Heart size={12} /> {t.likes}</span>
                      </div>
                    </div>
                    <div className="flex flex-col gap-2 shrink-0">
                      <button
                        onClick={(e) => { e.stopPropagation(); handleViewTrip(t.id); }}
                        className="w-8 h-8 rounded-full bg-blue-500/10 text-blue-400 flex items-center justify-center cursor-pointer"
                      >
                        <Eye size={14} />
                      </button>
                      <button
                        onClick={(e) => { e.stopPropagation(); handleDeleteTrip(t.id, t.title); }}
              ...
              <table className="w-full">
                <thead>
                  <tr className="text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest border-b border-white/[0.06]">
                    ...
                    <th className="px-4 py-4">Dní</th>
                    <th className="px-4 py-4 hidden lg:table-cell">Aktivity</th>
                    <th className="px-4 py-4">Výdaje</th>
                    <th className="px-4 py-4 hidden lg:table-cell">Likes</th>
                  ...
                  {trips.map((t, i) => (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-[12px] font-bold truncate max-w-[120px]">
                              {t.user.firstName ? `${t.user.firstName} ${t.user.lastName || ''}` : t.user.email.split('@')[0]}
                            </p>
                            <p className="text-[10px] text-gray-500 truncate max-w-[120px]">{t.user.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4 hidden lg:table-cell">
                        <span className="text-[13px] font-bold">{t.activityCount}</span>
                      </td>
                      <td className="px-4 py-4">
                        <span className="text-[13px] font-bold text-orange-400">{t.totalExpenses.toLocaleString('cs-CZ')} Kč</span>
                      </td>
                      <td className="px-4 py-4 hidden lg:table-cell">
                        <span className="text-[13px] font-bold flex items-center gap-1"><Heart size={12} className="text-red-400" /> {t.likes}</span>
                      </td>
                ...
                {detailLoading || selectedTrip?.loading ? (
                  ...
                  <div className="p-8 space-y-6">
                    ...
                    {selectedTrip.activities?.length > 0 && (
                      <div>
                        <h3 className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                          <MapPin size={13} /> Itinerář ({selectedTrip.activities.length} dní)
                        </h3>
                    ...
                    {selectedTrip.expenses?.length > 0 && (
                      <div>
                        <h3 className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                          <DollarSign size={13} /> Výdaje ({selectedTrip.expenses.length} položek · {totalTripExpenses(selectedTrip).toLocaleString('cs-CZ')} Kč)
                        </h3>
                            ...
                            return (
                              <div key={e.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-[10px] text-gray-500">{cat.label}{e.date ? ` · ${e.date}` : ''}</p>
                                </div>
                                <span className="text-[13px] font-bold text-orange-400 shrink-0">{e.amount.toLocaleString('cs-CZ')} Kč</span>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    {selectedTrip.packingList?.length > 0 && (
                      <div>
                        <h3 className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                          <Package size={13} /> Balící seznam ({selectedTrip.packingList.filter(p => p.checked).length}/{selectedTrip.packingList.length})
                        </h3>
                    ...
                    {selectedTrip.documents?.length > 0 && (
                      <div>
                        <h3 className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                          <FileText size={13} /> Poznámky ({selectedTrip.documents.length})
                        </h3>
                        <div className="space-y-2">
                    ...
                    {!selectedTrip.activities?.length && !selectedTrip.expenses?.length && !selectedTrip.packingList?.length && !selectedTrip.documents?.length && (
                      <div className="text-center py-12 text-gray-600">
                        ...
                        <p className="text-[13px]">Tento výlet zatím nemá žádný obsah.</p>
                      </div>
                    )}
