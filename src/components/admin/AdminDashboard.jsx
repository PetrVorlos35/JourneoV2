    ...
    <div className="space-y-8">
      ...
      <motion.div {...reveal()}>
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-balance">
          ...
          <span className="text-orange-500 dark:text-orange-400">{t('admin.dashboard.overview')}</span>
        </h1>
            ...
            <div key={i} className="bg-white dark:bg-[#0e0e10] p-5 sm:p-6">
              <div className="flex items-center gap-2 mb-3">
                <Icon
                  size={15}
                  strokeWidth={2.5}
                  className={k.primary ? 'text-orange-500 dark:text-orange-400' : 'text-gray-400 dark:text-gray-500'}
                />
              ...
              <p
                className={`text-2xl sm:text-3xl font-bold tracking-tight tabular-nums ${
                  k.primary ? 'text-orange-500 dark:text-orange-400' : 'text-gray-900 dark:text-white'
                }`}
              >
                {k.value}
              </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center gap-3 mb-6">
            <div className="w-9 h-9 rounded-xl bg-orange-500/10 flex items-center justify-center">
              <TrendingUp size={18} className="text-orange-500 dark:text-orange-400" strokeWidth={2} />
            </div>
            <div>
              <h3 className="font-bold text-lg">{t('admin.dashboard.tripsChart.title')}</h3>
              <p className="text-[12px] text-gray-500 dark:text-gray-400 font-medium">{t('admin.dashboard.tripsChart.subtitle')}</p>
            </div>
          </div>
              ...
              {data.tripsPerMonth.map((m, i) => {
                ...
                return (
                  <div key={m.month} className="flex-1 flex flex-col items-center gap-2">
                    <span className="text-[11px] font-bold text-gray-500 dark:text-gray-400 tabular-nums">{m.count}</span>
                    <motion.div
                      initial={shouldReduceMotion ? false : { height: 0 }}
                      animate={{ height: pct }}
                      transition={shouldReduceMotion ? { duration: 0 } : { duration: 0.6, delay: 0.2 + i * 0.08 }}
                      className="w-full min-h-[8px] rounded-xl bg-orange-500 dark:bg-orange-500/90"
                    />
                    <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase">{getMonthLabel(monthKey)}</span>
                  </div>
                );
              })}
            </div>
        ...
        >
          <div className="flex items-center gap-3 mb-6">
            <div className="w-9 h-9 rounded-xl bg-orange-500/10 flex items-center justify-center">
              <Users size={18} className="text-orange-500 dark:text-orange-400" strokeWidth={2} />
            </div>
            <div>
              <h3 className="font-bold text-lg">{t('admin.dashboard.topUsers.title')}</h3>
              <p className="text-[12px] text-gray-500 dark:text-gray-400 font-medium">{t('admin.dashboard.topUsers.subtitle')}</p>
            </div>
          </div>
          ...
          <div className="space-y-1">
            {data.topUsers.map((u, i) => (
              <div key={u.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-white/[0.03] transition-colors">
                <span className="text-[13px] font-bold text-gray-400 dark:text-gray-500 w-6 text-center tabular-nums">{i + 1}</span>
                ...
                <span className="text-[13px] font-bold text-orange-500 dark:text-orange-400 tabular-nums">{u.tripCount} {t('admin.dashboard.stats.tripCount')}</span>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          <div className="w-9 h-9 rounded-xl bg-orange-500/10 flex items-center justify-center">
            <UserPlus size={18} className="text-orange-500 dark:text-orange-400" strokeWidth={2} />
          </div>
          <div>
            <h3 className="font-bold text-lg">{t('admin.dashboard.recentUsers.title')}</h3>
            <p className="text-[12px] text-gray-500 dark:text-gray-400 font-medium">{t('admin.dashboard.recentUsers.subtitle')}</p>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="text-left text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-200 dark:border-white/[0.06]">
                ...
                <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="py-3 pr-4 hidden md:table-cell">
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full ${
                      u.role === 'admin' ? 'bg-orange-500/15 text-orange-600 dark:text-orange-400' : 'bg-gray-100 dark:bg-white/[0.06] text-gray-500 dark:text-gray-400'
                    }`}>
                      {u.role || 'user'}
                    </span>
                  </td>
                  <td className="py-3 pr-4">
                    <span className="text-[13px] font-bold tabular-nums">{u.tripCount}</span>
                  </td>
                  <td className="py-3">
